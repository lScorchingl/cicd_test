...
# АВАТАРКА НЕ МЕНЯЕТСЯ ПОФИКСИТЬ ЭТО. НЕ ПРОЖИМАЕТСЯ КНОПКА ОК ПРИ СМНЕ АВЫ
        ...
        ok_button = self.wait.until(EC.element_to_be_clickable((By.XPATH, "//button[contains(text(),'ОK')]")))
        ok_button.click()
    ...
    def change_password(self):
        ...
        WebDriverWait(self.browser, 10).until(EC.presence_of_element_located((By.XPATH, "//li[contains(text(),'Выход')]"))).click()
